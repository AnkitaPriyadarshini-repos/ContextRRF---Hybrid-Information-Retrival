...
question against its target project's ContextRRF index, and computes file-level
AND chunk-level precision/recall.  Produces a per-question detail report and
aggregate metrics.

Usage:
    cd /path/to/contextrrf-repo
    python3 -m contextrrf.tests.benchmark_suite
    python3 -m contextrrf.tests.benchmark_suite --questions-dir /path/to/questions
    python3 -m contextrrf.tests.benchmark_suite --budget 8000
"""
...
class BenchmarkSuite:
    ...
    def _run_project(self, project: BenchmarkProject) -> ProjectResult:
        ...
        # Set up ContextRRF for this project
        engine, config = self._setup_contextrrf(project.root)
        print("  ContextRRF initialised")
    ...
    def _setup_contextrrf(self, project_root: str) -> tuple:
        """
        Initialise ContextRRF for a project and return (engine, config).
        ...
        contextrrf_dir = os.path.join(project_root, ".contextrrf")

        # Clean slate for reproducibility
        db_path = os.path.join(contextrrf_dir, "contextrrf.db")
        bloom_path = os.path.join(contextrrf_dir, "bloom.bin")
        for f in (db_path, bloom_path):
            if os.path.isfile(f):
                os.remove(f)
        os.makedirs(contextrrf_dir, exist_ok=True)
        ...
        conn = open_store(contextrrf_dir)
        store = Store(conn)
        bloom = BloomFilter()
        tfidf = get_backend(config, store)
        audit = AuditLog(os.path.join(contextrrf_dir, "audit.log"))
        ...
        try:
            ...
            if DenseBackend.is_available():
                ...
                dense_backend = DenseBackend(
                    config, store,
                    model_dir=os.path.join(contextrrf_dir, "models"),
                )
        except Exception:
            pass
...
def discover_projects(
    ...
    Args:
        ...
        base_dir:      Base directory for resolving relative ``root`` paths.
                       Defaults to the parent of the contextrrf package.

    Returns:
        List of :class:`BenchmarkProject` descriptors.
    """
    if base_dir is None:
        # Default: the directory containing the contextrrf package
