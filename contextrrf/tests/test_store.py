...
def _chunk(file_id, content="def foo():\n    pass\n", chunk_type="function"):
    from contextrrf.hasher import content_hash
...
class TestStore(unittest.TestCase):
    ...
    def test_record_usage_and_get_scores(self):
        ...
        from contextrrf.models import UsageEvent
        event = UsageEvent(
            event_id=None,
            chunk_id=cid,
    ...
    def test_save_and_load_cache_state(self):
        ...
        from contextrrf.models import CacheEntry
    ...
    def test_concurrent_upsert_files(self):
        ...
        def _worker(thread_id: int) -> None:
            try:
                ...
                conn = get_connection(Path(tmp) / "contextrrf.db")
                init_db(conn)
                store = Store(conn)
