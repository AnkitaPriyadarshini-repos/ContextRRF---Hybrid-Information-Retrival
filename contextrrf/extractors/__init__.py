...
"""
Document extractor registry and dispatch for ContextRRF.
...
from contextrrf.extractors.base import ExtractedContent
...
from contextrrf.extractors import (
    csv_extractor,
    docx_extractor,
    plaintext_extractor,
)
...
def get_extractor(file_path: str, config: "Config" = None) -> object | None:
    ...
    Extractor priority for PDF:
      1. pypdf (direct text) -- available with ``contextrrf-engine[pdf]``
      2. None (no OCR in Tier 0)

    Args:
        file_path: Path to the file to extract.
        config:    Optional ContextRRF config (reserved for future
    ...
    if ext == ".pdf":
        from contextrrf.extractors import pdf_extractor
...
def extract_file(file_path: str, config: "Config" = None) -> ExtractedContent | None:
    ...
    Args:
        ...
        config:    Optional ContextRRF config.
