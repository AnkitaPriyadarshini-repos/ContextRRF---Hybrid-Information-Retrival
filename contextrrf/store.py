...
"""
Repository layer for ContextRRF -- all SQLite CRUD lives here.
...
class Store:
    """
    Repository providing typed access to all ContextRRF database tables.

    Args:
        conn: An open :class:`sqlite3.Connection`, typically obtained from
              :func:`contextrrf.schema.get_connection` after :func:`contextrrf.schema.init_db`.
    """
    ...
    def __init__(self, conn: sqlite3.Connection) -> None:
        ...
        if db_path is not None:
            self._lock_path = db_path.parent / "contextrrf.lock"
    ...
    def _with_write_lock(self) -> Iterator[None]:
        ...
        The lock file lives alongside the database (``<db_dir>/contextrrf.lock``).
    ...
    def get_usage_events(
        ...
        Returns:
            List of :class:`~contextrrf.models.UsageEvent` objects.
        """
        if event_types:
