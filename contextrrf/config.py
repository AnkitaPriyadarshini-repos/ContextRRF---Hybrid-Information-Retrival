...
"""
Configuration loader for ContextRRF.

Reads from `.contextrrf/config.toml` (relative to project root) if present,
...
try:
    ...
    try:
        ...
        tomllib = _TomlWrapper()  # type: ignore
...
DEFAULTS: dict[str, dict[str, Any]] = {
    "general": {
        "project_root": ".",
        "ignore_patterns": [
            # Hidden directories and files -- catches .git, .contextrrf,
...
class Config:
    ...
    ``<root>/.contextrrf/config.toml`` and deep-merge any values found there
    on top of DEFAULTS.
    ...
    _TOML_REL = os.path.join(".contextrrf", "config.toml")

    def __init__(self, root: str | os.PathLike | None = None) -> None:
        if root is None:
            root = os.getcwd()
        self._root = Path(root).resolve()
        self._config_path = self._root / ".contextrrf" / "config.toml"
    ...
    def save(self) -> None:
        """Persist the current configuration to ``.contextrrf/config.toml``."""
