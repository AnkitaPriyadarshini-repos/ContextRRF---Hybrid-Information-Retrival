...
"""
ContextRRF command-line interface.

Entry point: ``contextrrf`` (configured in pyproject.toml / setup.py as
``contextrrf.cli:main``).

Commands
--------
init      -- Create ``.contextrrf/`` and default config in the current directory.
...
def _configure_logging(log_format: str, log_level: str) -> None:
    ...
    root = logging.getLogger("contextrrf")
    root.handlers.clear()
...
def _find_project_root() -> str:
    """
    Walk upward from cwd looking for a ``.contextrrf/`` directory.

    Returns the directory containing ``.contextrrf/``, or cwd if none found.
    """
    current = Path(os.getcwd()).resolve()
    for parent in [current, *current.parents]:
        if (parent / ".contextrrf").is_dir():
            return str(parent)
    return str(current)


def _open_store_conn(project_root: str):
    """Open (and initialise if needed) the ContextRRF SQLite connection."""
    from contextrrf.schema import open_store
    db_dir = os.path.join(project_root, ".contextrrf")
...
def _check_upgrade_hint(store) -> None:
    ...
    if pending is None or pending == "":
        return
    import sys
    from contextrrf import __version__
    print(
        f"[contextrrf] Upgraded to v{__version__} -- documents now indexed separately.\n"
        f"[contextrrf] Use --all to search code + docs, --docs for documents only.",
        file=sys.stderr,
    )
...
def _make_bloom(project_root: str):
    ...
    bloom_path = os.path.join(project_root, ".contextrrf", "bloom.bin")
...
def _make_audit(project_root: str):
    ...
    audit_path = os.path.join(project_root, ".contextrrf", "audit.log")
    return AuditLog(audit_path)


def _make_analytics(store, config):
    from contextrrf.analytics import Analytics
    return Analytics(store, config)


def _make_prefetcher(store):
    from contextrrf.prefetch import Prefetcher
...
def _make_dense_backend(config, store, project_root: str):
    ...
    if not dense_model:
        return None
    try:
        from contextrrf.embeddings.dense_backend import DenseBackend
        model_dir = os.path.join(project_root, ".contextrrf", "models")
...
def _make_cache(config):
    from contextrrf.cache import ARCCache
    return ARCCache(
...
def _require_contextrrf_dir(project_root: str) -> bool:
    """Return True if .contextrrf exists; print an error and return False otherwise."""
    if not os.path.isdir(os.path.join(project_root, ".contextrrf")):
        print(
            "No .contextrrf/ directory found. Run 'contextrrf init' first.",
            file=sys.stderr,
        )
        return False
    return True
...
def cmd_init(args: argparse.Namespace) -> int:
    """
    Initialise a ``.contextrrf/`` workspace in the current directory.

    Creates:
    - ``.contextrrf/`` directory
    - ``.contextrrf/config.toml`` (default values)
    - ``.contextrrf/contextrrf.db`` (schema applied)
    """
    target_dir = Path(os.getcwd()).resolve()
    contextrrf_dir = target_dir / ".contextrrf"

    if contextrrf_dir.exists():
        print(f"Already initialised: {contextrrf_dir}")
        return 0

    contextrrf_dir.mkdir(parents=True, exist_ok=True)

    # Write default config
    from contextrrf.config import Config
    config = Config(root=str(target_dir))
    config.save()
    print(f"Created: {contextrrf_dir / 'config.toml'}")

    # Initialise database
    try:
        conn = _open_store_conn(str(target_dir))
        conn.close()
        print(f"Created: {contextrrf_dir / 'contextrrf.db'}")
    except Exception as exc:
        print(f"Warning: could not create database: {exc}", file=sys.stderr)

    print(f"\nContextRRF initialised at: {target_dir}")
    print("Run 'contextrrf ingest' to index your project.")
    return 0
...
def cmd_ingest(args: argparse.Namespace) -> int:
    ...
    if not _require_contextrrf_dir(project_root):
        return 1
    ...
    from contextrrf.embeddings import get_backend as _get_be
...
def cmd_schema_ingest(args: argparse.Namespace) -> int:
    ...
    from contextrrf.schema_ingest import SchemaIngester
...
def cmd_query(args: argparse.Namespace) -> int:
    ...
    from contextrrf.formatter import Formatter
    ...
    if search_docs or search_all:
        from contextrrf.doc_store import DocStore
        from contextrrf.doc_retrieval import DocRetrievalEngine
        from contextrrf.embeddings import get_backend as _get_be
...
def cmd_compress(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        from contextrrf.models import Chunk, estimate_tokens
...
def cmd_cache(args: argparse.Namespace) -> int:
    ...
    if action == "show":
        ...
        from contextrrf.tiers import TierManager
...
def cmd_delta(args: argparse.Namespace) -> int:
    ...
    from contextrrf.delta import DeltaTracker
...
def cmd_benchmark(args: argparse.Namespace) -> int:
    ...
    from contextrrf.tests.benchmark_suite import BenchmarkSuite, discover_projects

    questions_dir = args.questions_dir
    if questions_dir is None or not os.path.isdir(questions_dir):
        # Try relative to contextrrf package
...
def cmd_daemon(args: argparse.Namespace) -> int:
    """Manage the ContextRRF background daemon."""
    ...
    if action == "start":
        ...
        from contextrrf.daemon import is_daemon_alive
        if is_daemon_alive(project_root):
            from contextrrf.daemon import read_pid
            print(f"Daemon already running (PID {read_pid(project_root)}).")
            return 0

        if args.foreground:
            from contextrrf.daemon import ContextRRFDaemon
            daemon = ContextRRFDaemon(project_root)
            daemon.start()
        else:
            pid = os.fork()
            if pid == 0:
        ...
        from contextrrf.daemon import read_pid, is_daemon_alive
        ...
        if not is_daemon_alive(project_root):
            ...
            pid_path = os.path.join(project_root, ".contextrrf", "daemon.pid")
            try:
                os.unlink(pid_path)
            except OSError:
                pass
...
def cmd_health(args: argparse.Namespace) -> int:
    ...
    # -- Daemon status ---------------------------------------------------
    try:
        from contextrrf.daemon import is_daemon_alive, read_pid
        pid = read_pid(project_root)
...
def build_parser() -> argparse.ArgumentParser:
    """Build and return the argument parser for the ``contextrrf`` CLI."""
    parser = argparse.ArgumentParser(
        prog="contextrrf",
        description="ContextRRF -- LLM Context Compression & Retrieval Engine",
    ...
    sub.add_parser(
        "init",
        help="Initialise .contextrrf/ in the current directory",
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"Error: {exc}", file=sys.stderr)
        if os.environ.get("CONTEXTRRF_DEBUG"):
            import traceback
            traceback.print_exc()
        return 1
