"""ContextRRF MCP Server.

Exposes ContextRRF's 6-signal hybrid code retrieval engine as MCP tools
...
# ---------------------------------------------------------------------------
# Lazy engine initialization -- only imports contextrrf when first tool is called
# ---------------------------------------------------------------------------
...
def _get_engine(project_root: str) -> tuple:
    ...
    contextrrf_dir = Path(resolved) / ".contextrrf"
    if not contextrrf_dir.exists():
        from contextrrf.schema import init_db

        contextrrf_dir.mkdir(parents=True, exist_ok=True)
        init_db(str(contextrrf_dir))

    config = Config(root=resolved)
    conn = open_store(str(contextrrf_dir))
    store = Store(conn)
...
def _resolve_project_root(requested: str | None) -> str:
    ...
    env_root = os.environ.get("CONTEXTRRF_PROJECT_ROOT")
    if env_root:
...
async def list_tools() -> list[Tool]:
    return [
        Tool(
            name="search",
            description=(
                "Search a codebase using ContextRRF's 6-signal hybrid retrieval engine. "
                ...
                "properties": {
                    ...
                    "project_root": {
                        ...
                        "description": (
                            ...
                            "Defaults to CONTEXTRRF_PROJECT_ROOT env var or cwd."
                        ),
                    },
                },
                "required": ["query"],
        ...
        Tool(
            name="index",
            description=(
                "Index or re-index a codebase for ContextRRF retrieval. "
        ...
        Tool(
            name="stats",
            description=(
                "Show index statistics for a ContextRRF-indexed codebase: "
        ...
        Tool(
            name="search_docs",
            description=(
                "Search the document partition of a ContextRRF index. Searches "
        ...
        Tool(
            name="schema_ingest",
            description=(
                "Ingest database schema into the ContextRRF index. Accepts DDL files, "
...
async def _handle_index(arguments: dict) -> list[TextContent]:
    ...
    from contextrrf.config import Config
    from contextrrf.schema import open_store, init_db
    from contextrrf.store import Store
    from contextrrf.embeddings import get_backend
    ...
    contextrrf_dir = Path(project_root) / ".contextrrf"
    if not contextrrf_dir.exists():
        contextrrf_dir.mkdir(parents=True, exist_ok=True)
        init_db(str(contextrrf_dir))
    ...
    tfidf = get_backend(config, store)
    bloom = BloomFilter()
    audit = AuditLog(str(contextrrf_dir / "audit.jsonl"))
...
async def _handle_stats(arguments: dict) -> list[TextContent]:
    ...
    lines = [
        f"ContextRRF Index: {project_root}",
        f"  Files:    {file_count}",
...
async def main() -> None:
    """Run the ContextRRF MCP server over stdio."""
