...
def _get_store(project_root: Path) -> Store:
    ...
    contextrrf_dir = project_root / ".contextrrf"
    contextrrf_dir.mkdir(parents=True, exist_ok=True)
    db_path = contextrrf_dir / "contextrrf.db"
...
def _make_ingester(project_root: Path):
    ...
    bloom_path = contextrrf_dir / "bloom.bin"
    ...
    audit = AuditLog(contextrrf_dir / "audit.jsonl")
    return Ingester(
