...
path containment security, ContextRRF DB rejection, FTS5 searchability
...
def _setup_project():
    """Create a temp project with .contextrrf/ initialized."""
    ...
    db_dir = os.path.join(tmpdir, ".contextrrf")
...
class TestSqliteIntrospectionSecurity(unittest.TestCase):
    ...
    def test_rejects_contextrrf_own_database(self):
        contextrrf_db = os.path.join(self.tmpdir, ".contextrrf", "contextrrf.db")
        with self.assertRaises(ValueError) as ctx:
            self.ingester.introspect_sqlite(contextrrf_db, env_tag="dev")
