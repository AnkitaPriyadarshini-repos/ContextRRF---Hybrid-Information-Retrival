...
from contextrrf_ollama.bridge import McpBridge
...
def _log_partitions(result_text: str, tool_name: str, out) -> None:
    ...
    if tool_name == "search_docs":
        ...
        print(f"[contextrrf-ollama] results: docs: {chunks} chunks", file=out)
        if files:
            preview = ", ".join(files[:5])
            if len(files) > 5:
                preview += f", ... (+{len(files) - 5})"
            print(f"[contextrrf-ollama]   docs: {preview}", file=out)
        return
    ...
    print(f"[contextrrf-ollama] results: {' | '.join(parts)}", file=out)
    ...
    for label, files in [("code", code_files), ("docs", doc_files)]:
        if files:
            ...
            if len(files) > 5:
                preview += f", ... (+{len(files) - 5})"
            print(f"[contextrrf-ollama]   {label}: {preview}", file=out)
...
SYSTEM_PROMPT = """\
You are a code search assistant powered by ContextRRF. Answer questions about \
...
async def run(
    ...
    *,
    ...
    """Run a single query through Ollama with contextrrf-mcp tools.
    ...
    Args:
        ...
        budget: Token budget for contextrrf search results.
    ...
    if not model:
        ...
        if verbose:
            print(f"[contextrrf-ollama] auto-detected model: {model}", file=sys.stderr)
    ...
    try:
        ...
        return AgentResult(
            response="",
            error="contextrrf-mcp not found on PATH. Install: pip install contextrrf-mcp",
        )
    ...
    try:
        ...
        for step in range(max_steps):
            ...
            for tc in tool_calls:
                ...
                if verbose:
                    print(
                        f"[contextrrf-ollama] tool: {name}({json.dumps(arguments, indent=None)})",
                        file=sys.stderr,
                    )
