...
"""
Schema ingestion orchestrator for ContextRRF.

Reads database schema from DDL files, JSON snapshots, or YAML snapshots and
ingests them as first-class chunks in the ContextRRF index.  Schema sources are
...
class SchemaIngester:
    """Ingest database schema sources into the ContextRRF index.
    ...
    Args:
        ...
        config:       ContextRRF Config instance.
    ...
    def introspect_sqlite(
        ...
        # Security: reject ContextRRF's own database
        contextrrf_db = os.path.join(self.root, ".contextrrf", "contextrrf.db")
        if os.path.abspath(abs_db) == os.path.abspath(contextrrf_db):
            raise ValueError("Cannot introspect ContextRRF's own database")
    ...
    def _ingest_content(
        ...
        from contextrrf.chunkers import get_chunker
