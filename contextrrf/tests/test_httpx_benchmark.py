...
Clones httpx 0.28.1 (cached), indexes it with ContextRRF, runs 6 retrieval
questions, and asserts that key metrics do not regress below the established
baseline.

Run with:
    python3 -m pytest contextrrf/tests/test_httpx_benchmark.py -m benchmark -v
"""
...
from contextrrf.tests.benchmark_suite import BenchmarkSuite
...
def httpx_benchmark_results(httpx_root):
    ...
    # Setup contextrrf on the httpx project
    suite = BenchmarkSuite([])
    engine, config = suite._setup_contextrrf(httpx_root)
