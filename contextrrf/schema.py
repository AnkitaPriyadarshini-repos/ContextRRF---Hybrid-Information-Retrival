...
"""
SQLite schema manager for ContextRRF.
...
def init_db(conn: sqlite3.Connection) -> None:
    """
    Create all ContextRRF tables, indexes, triggers, and FTS5 virtual tables.
...
def open_store(db_dir: str | Path) -> sqlite3.Connection:
    """
    Convenience helper: ensure *db_dir* exists, open the ContextRRF database
    inside it, initialise the schema, and return the connection.

    The database file will be at ``<db_dir>/contextrrf.db``.
    ...
    conn = get_connection(db_dir / "contextrrf.db")
    init_db(conn)
    return conn
