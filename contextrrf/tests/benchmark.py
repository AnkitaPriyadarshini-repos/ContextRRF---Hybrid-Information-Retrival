...
"""
ContextRRF Benchmark -- single-project edition.

Measures token reduction, retrieval precision, compression ratios, query
speed, and storage overhead for the ContextRRF context engine against a
target project.

Usage:
    python -m contextrrf.tests.benchmark --project-root /path/to/project
...
# ---------------------------------------------------------------------------
# Path bootstrap -- make sure the contextrrf package is importable when this
...
_SKIP_DIRS = {".contextrrf", "node_modules", ".git", "contextrrf", "__pycache__"}
...
class ContextRRFBenchmark:
    """End-to-end benchmark harness for ContextRRF against a target project."""
    ...
    def __init__(
        ...
        self.budget = budget
        self.contextrrf_dir = os.path.join(self.project_root, ".contextrrf")
    ...
    def setup(self) -> dict:
        """Initialize ContextRRF and ingest the project. Returns ingest stats."""
        ...
        db_path = os.path.join(self.contextrrf_dir, "contextrrf.db")
        bloom_path_init = os.path.join(self.contextrrf_dir, "bloom.bin")
        for f in (db_path, bloom_path_init):
            if os.path.isfile(f):
                os.remove(f)
        os.makedirs(self.contextrrf_dir, exist_ok=True)
        ...
        self.conn = open_store(self.contextrrf_dir)

        # 4. Build auxiliary components
        self.store = Store(self.conn)
        bloom = BloomFilter()

        bloom_path = os.path.join(self.contextrrf_dir, "bloom.bin")
        ...
        audit = AuditLog(os.path.join(self.contextrrf_dir, "audit.log"))
    ...
    def measure_token_reduction(self) -> list[dict]:
        """For each question: count raw tokens vs contextrrf tokens."""
    ...
    def measure_storage(self) -> dict:
        """Compare raw source size vs .contextrrf/ size."""
        source_files = self._get_source_files()
        raw_size = sum(os.path.getsize(f) for f in source_files if os.path.isfile(f))

        db_size = 0
        if os.path.isdir(self.contextrrf_dir):
            for fname in os.listdir(self.contextrrf_dir):
                fp = os.path.join(self.contextrrf_dir, fname)
    ...
    def format_report(self, results: dict) -> str:
        ...
        lines.append(sep)
        lines.append("  CONTEXTRRF BENCHMARK REPORT")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="ContextRRF Benchmark -- measures token reduction, retrieval precision, "
    ...
    bench = ContextRRFBenchmark(args.project_root, args.budget)

    print("Setting up ContextRRF...")
