...
"""
Dense embedding backend for ContextRRF.
...
def _download_file(
    ...
    req = urllib.request.Request(
        url,
        headers={"User-Agent": "contextrrf-engine/1.0"},
    )
...
class DenseBackend:
    ...
    Downloads the model on first use (lazy, cached in .contextrrf/models/).
    ...
    Args:
        ...
        model_dir: Directory to cache model files.  Defaults to
                   .contextrrf/models/ under the project root.
    """
    ...
    def __init__(
        ...
        self._model_dir = model_dir or os.path.join(
            str(getattr(config, "_root", ".")), ".contextrrf", "models"
        )
