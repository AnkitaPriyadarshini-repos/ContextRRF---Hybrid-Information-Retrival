...
"""
Hot/warm/cold tier manager for ContextRRF.
...
if TYPE_CHECKING:
    from contextrrf.models import Chunk
...
class TierManager:
    ...
    * **hot**  -- live in the in-memory :class:`~contextrrf.cache.ARCCache`.
    * **warm** -- on disk in the :class:`~contextrrf.store.Store`, but recently
    ...
    * **cold** -- on disk, not recently accessed.  Fetching a cold chunk
    ...
    Args:
        cache: The :class:`~contextrrf.cache.ARCCache` instance.
        store: The persistent :class:`~contextrrf.store.Store` instance.
    """
    ...
    def get(self, chunk_id: int) -> "Chunk | None":
        ...
        Returns:
            The :class:`~contextrrf.models.Chunk`, or ``None`` if not found
            anywhere.
        """
