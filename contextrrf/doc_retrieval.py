...
"""
Document retrieval engine for ContextRRF.
...
Reuses :func:`~contextrrf.ranking.rrf_fuse` and
:func:`~contextrrf.ranking.budget_cut` from the shared ranking module.
"""
...
from contextrrf.models import QueryResult, Chunk, estimate_tokens
from contextrrf.ranking import rrf_fuse, budget_cut

if TYPE_CHECKING:
    from contextrrf.doc_store import DocStore
    from contextrrf.config import Config
    from contextrrf.embeddings.tfidf_backend import TFIDFBackend
...
class DocRetrievalEngine:
    ...
    Args:
        doc_store:     :class:`~contextrrf.doc_store.DocStore` instance.
        tfidf_backend: TF-IDF backend trained on document vocabulary.
        config:        ContextRRF config.
    """
    ...
    def query(
        ...
        Returns:
            Ordered list of :class:`~contextrrf.models.QueryResult`.
        """
    ...
    def _get_file_record(self, file_id: int):
        ...
        if row is None:
            return None
        from contextrrf.models import FileRecord
        keys = row.keys()
        return FileRecord(
            file_id=row["file_id"],
