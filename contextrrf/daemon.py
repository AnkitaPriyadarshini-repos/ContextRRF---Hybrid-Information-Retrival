...
"""
JSON-RPC daemon for ContextRRF.
...
Protocol: client connects to ``.contextrrf/contextrrf.sock``, sends
...
logger = logging.getLogger("contextrrf.daemon")
_MAX_REQUEST_BYTES = 65_536  # 64 KiB cap on request payload


class ContextRRFDaemon:
    """Persistent JSON-RPC server over a Unix domain socket."""

    def __init__(self, project_root: str) -> None:
        self.project_root = os.path.abspath(project_root)
        self._contextrrf_dir = os.path.join(self.project_root, ".contextrrf")
        self._socket_path = os.path.join(self._contextrrf_dir, "contextrrf.sock")
        self._pid_path = os.path.join(self._contextrrf_dir, "daemon.pid")
    ...
    def _init_components(self) -> None:
        ...
        self.conn = open_store(self._contextrrf_dir)
    ...
    def start(self) -> None:
        ...
        os.makedirs(self._contextrrf_dir, exist_ok=True)
    ...
    def _handle_ingest(self, params: dict) -> dict:
        ...
        bloom_path = os.path.join(self._contextrrf_dir, "bloom.bin")
        try:
        ...
        ingester = Ingester(
            ...
            audit=AuditLog(os.path.join(self._contextrrf_dir, "audit.log")),
        )
