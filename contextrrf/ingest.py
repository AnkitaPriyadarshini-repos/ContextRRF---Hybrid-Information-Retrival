...
"""
Ingestion orchestrator for ContextRRF.
...
from contextrrf.models import FileRecord, Chunk, estimate_tokens
...
class Ingester:
    ...
    Args:
        ...
        config:        ContextRRF :class:`~contextrrf.config.Config` instance.
        store:         Persistent :class:`~contextrrf.store.Store` instance.
        bloom:         :class:`~contextrrf.bloom.BloomFilter` for fast
                       "already indexed" checks.
        tfidf_backend: TF-IDF backend with a ``fit(chunk_id, text)``
                       method for updating the embedding index.
        audit:         :class:`~contextrrf.audit.AuditLog` for operation
                       provenance records.
    """
    ...
    def _resolve_paths(self, paths: list[str]) -> list[str]:
        ...
        """
        from contextrrf.hasher import is_document
    ...
    def _needs_indexing(self, abs_path: str, rel_path: str, full: bool) -> bool:
        ...
        * The file mtime or content hash differs from the stored record.
        ...
        if mtime != file_record.last_modified:
            ...
            try:
                from contextrrf.hasher import file_hash
    ...
    def _index_file(self, abs_path: str, rel_path: str) -> tuple[int, int]:
        ...
        """
        from contextrrf.hasher import file_hash, is_binary, is_document
        from contextrrf.chunkers import get_chunker, detect_language
        ...
        for cand in candidates:
            from contextrrf.hasher import content_hash as compute_content_hash
    ...
    def _index_document(self, abs_path: str, rel_path: str) -> tuple[int, int]:
        ...
        from contextrrf.extractors import extract_file
        from contextrrf.hasher import file_hash_binary, content_hash as compute_content_hash
        from contextrrf.chunkers.document_chunker import DocumentChunker
